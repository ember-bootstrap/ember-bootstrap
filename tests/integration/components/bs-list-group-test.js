...
module('Integration | Component | bs-list-group', function (hooks) {
  ...
  test('bs-list-group has correct default markup', async function (assert) {
    ...
    assert.dom('ul').exists('ul exists').hasClass('list-group', 'ul has list-group class');
  });
  ...
  test('bs-list-group can be borderless', async function (assert) {
    ...
    assert.dom('ul').hasClass('list-group-flush', 'ul has list-group-flush class');
  });

  test('bs-list-group can be horizontal', async function (assert) {
    await render(hbs`<BsListGroup @horizontal={{true}} />`);

    assert.dom('ul').hasClass('list-group-horizontal', 'ul has list-group-horizontal class');
  });

  test('bs-list-group can be horizontal for selected screen size', async function (assert) {
    await render(hbs`<BsListGroup @horizontal={{true}} @horizontalSize="sm" />`);

    assert
      .dom('ul')
      .doesNotHaveClass('list-group-horizontal', 'ul does not have list-group-horizontal class')
      .hasClass('list-group-horizontal-sm', 'ul has list-group-horizontal-sm class');
  });
});
