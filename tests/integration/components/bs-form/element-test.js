...
module('Integration | Component | bs-form/element', function (hooks) {
  ...
  module('controlType "radio" is supported', function (hooks) {
    ...
    testBS45('has correct markup', async function (assert) {
      await render(
    ...
    testBS45('supports horizontal from layout', async function (assert) {
      await render(hbs`
    ...
    testBS45('supports inline', async function (assert) {
      await render(hbs`
        <BsForm::Element @controlType="radio" @options={{this.simpleOptions}} as |Element|>
          <Element.control @inline={{true}} />
        </BsForm::Element>
  ...
  testBS45RequiringFocus('event triggered on input group button does not enable validation', async function (assert) {
    this.set('errors', A(['Invalid']));
    this.set('model', EmberObject.create({ name: null }));
    await render(hbs`
      <BsForm as |form|>
  ...
  testBS45('support size classes', async function (assert) {
