...
module('Integration | Component | bs-navbar/nav', function (hooks) {
  ...
  Nav
    ...
    assert
      ...
      .exists(
        { count: 1 },
        'there is only one element with the navbar-nav class',
      );
    assert
      .dom('.navbar-nav')
      .hasClass(
        'nav',
        'it has the nav class indicating it derives from bs-nav',
      );
  });
  ...
  test('it no longer supports the justified option', async function (assert) {
    ...
    assert
      .dom('.navbar-justified')
      .doesNotExist('the justified class was not applied');
  });
