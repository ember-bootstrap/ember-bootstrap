...
module('Integration | Component | bs-tab/pane', function (hooks) {
  ...
  test('it has correct markup', async function (assert) {
    await render(hbs`<BsTab::Pane @fade={{true}} @activeId='pane1' @id='pane1'>
  template block text
</BsTab::Pane>`);
    ...
    assert
      .dom('div.tab-pane')
      .hasClass(visibilityClass(), 'tab-pane has visibility class');
  });

  test('it has correct markup when switching active pane and fade=false', async function (assert) {
    this.set('activeId', null);
    await render(hbs`<BsTab::Pane @fade={{false}} @activeId={{this.activeId}} @id='pane1'>
  template block text
</BsTab::Pane>`);

    assert
      .dom('div.tab-pane')
      .hasNoClass('fade', 'tab-pane does not have fade class');

    assert
      .dom('div.tab-pane')
      .hasNoClass('active', 'tab-pane does not have active class');
    assert
      .dom('div.tab-pane')
  ...
  testRequiringTransitions(
    ...
    async function (assert) {
      this.set('activeId', null);
      await render(hbs`<BsTab::Pane @fade={{true}} @activeId={{this.activeId}} @id='pane1'>
  template block text
</BsTab::Pane>`);
      ...
      assert
        ...
        .hasNoClass(
          visibilityClass(),
          'tab-pane does not have visibility class',
        );
      ...
      assert
        .dom('div.tab-pane')
        .hasClass('active', 'tab-pane has active class');
      assert
        .dom('div.tab-pane')
        .hasClass(visibilityClass(), 'tab-pane has visibility class');
