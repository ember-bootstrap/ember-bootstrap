...
module('Integration | Component | bs-popover', function (hooks) {
  ...
  skip('it supports different placements', async function (assert) {
    ...
    for (const placement of placements) {
      ...
      const placementClass = popoverPositionClass(placement);
      assert
        .dom('.popover')
  ...
  test('should adjust popover arrow', async function (assert) {
    ...
    const expectedArrowPosition = 217;
      ...
      <template>
        <div id='target'>
          <BsPopover
            ...
            as |po|
          >
            <div id='hide' {{on 'click' po.close}} role='button'>Hide</div>
          </BsPopover>
        </div>
      </template>,
    );
  ...
  test('non-click-initiated close action does not interfere with click-to-open', async function (assert) {
    await render(
      <template>
        <div id='target'><BsPopover as |po|><input
              id='hide'
              {{on 'blur' po.close}}
            /></BsPopover></div>
      </template>,
  ...
  test('non-click-initiated close action does not interfere with click-to-open when wormholed', async function (assert) {
    await render(
      <template>
        <div id='ember-bootstrap-wormhole'></div><div id='target'><BsPopover
            as |po|
          ><input id='hide' {{on 'blur' po.close}} /></BsPopover></div>
      </template>,
    );
    await click('#target');
    assert.dom('.popover').exists('popover is visible');
  ...
  test("Renders in wormhole's default destination if renderInPlace is not set", async function (assert) {
    ...
    await render(
      <template>
        <div id='ember-bootstrap-wormhole'></div>{{#if state.show}}<BsPopover
  ...
  test('it works with hover trigger and delayShow', async function (assert) {
    await render(
      <template>
        <div id='target'>
  ...
  test('it handles hover with delayShow when user leaves before rendering completes', async function (assert) {
    await render(
      <template>
        <div id='target'>
          <BsPopover @delayShow={{200}} @triggerEvents='hover'>
    ...
    // Hover and leave before delay completes - this is the race condition case
    triggerEvent('#target', 'mouseenter');
    await delay(10);
    triggerEvent('#target', 'mouseleave');
    await delay(10);
    assert
      .dom('.popover')
      .doesNotExist('popover is not visible after early leave');

    // Verify it still works correctly after the race condition
    triggerEvent('#target', 'mouseenter');
    await delay(210);
    assert.dom('.popover').exists('popover is now visible');
  });
